...
/*   Updated: 2024/07/12 21:47:23 by poss             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (!mlx.mlx || !mlx.window)
		cleanup_and_exit((t_data){&game, &mlx, NULL, NULL},
			"Error building window");
	sprites = load_sprites(mlx);
	if (sprites_are_invalid(sprites))
		cleanup_and_exit((t_data){&game, &mlx, &sprites, NULL},
			"Error loading sprites");
	render_input = build_render_input(&mlx, &game, &sprites, TILE_SIZE);
	update_input = build_update_input(&mlx, &game, &render_input);
	...
	mlx_loop(mlx.mlx);
	cleanup_and_exit((t_data){&game, &mlx, &sprites, &render_input.background},
		NULL);
}
